...
%     cfg.analysis = 'ROI'; % standard alternatives: 'wholebrain', 'ROI' (pass ROIs in cfg.files.mask, see below)
    ...
    if length(num2str(subj))==1
        ...
        cfg.results.dir = ['D:\MVPA_passive_movement\MVPA_results\Exp1\Direction\1stLevel\S0' num2str(subj)];
    else
        subj_folder=[folderName '\Data\S' num2str(subj)]; % subject folder
        cfg.results.dir = ['D:\MVPA_passive_movement\MVPA_results\Exp1\Direction\1stLevel\S' num2str(subj)];
    end
