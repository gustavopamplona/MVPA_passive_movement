% MVPA Passive Movement Exp 2
% Gustavo Pamplona, 01.09.2022
...
%     cfg.analysis = 'ROI'; % standard alternatives: 'wholebrain', 'ROI' (pass ROIs in cfg.files.mask, see below)
    ...
    if length(num2str(subj))==1
        ...
        cfg.results.dir = ['D:\MVPA_passive_movement\MVPA_results\Exp2\Amplitude\1stLevel\S0' num2str(subj)];
    else
        subj_folder=[folderName '\Data\S' num2str(subj)]; % subject folder
        cfg.results.dir = ['D:\MVPA_passive_movement\MVPA_results\Exp2\Amplitude\1stLevel\S' num2str(subj)];
    end

    beta_loc = [subj_folder '\1stLevel_unsmoothed'];

    cfg.files.mask = ['D:\MVPA_passive_movement\Analysis\mask_lowerRes.nii'];
%     cfg.files.mask = 'D:\MVPA_passive_movement\MVPA_results\Exp2\Amplitude\ROI\Rolandic_Oper_R_sphere_4-45_-19_20_roi.nii';
...
%     cfg.results.output = {'confusion_matrix'}; % 'accuracy_minus_chance' by default
    cfg.results.output = {'accuracy_minus_chance', 'model_parameters'}; % 'accuracy_minus_chance' by default
